...
def TCN(train_x, train_y, valid_x, valid_y):
    ...
    pruning_schedule = tfmot.sparsity.keras.PolynomialDecay(
        initial_sparsity=0.0,  # 初始剪枝比例
        final_sparsity=0.5,  # 最终剪枝比例
        begin_step=0,  # 训练开始时应用剪枝
        end_step=2000,  # 训练结束时剪枝的最大比例
        frequency=100  # 每100个训练步骤执行一次剪枝
    )
    ...
    # 同时在验证集上监控模型性能
    model.fit(train_x, train_y, batch_size=64, epochs=100, verbose=2, validation_data=(valid_x, valid_y), callbacks=callbacks)

    # 训练完成后，将模型保存到指定文件中
    model.save('./model/E-TCN-PPA-final-pruned.h5')
