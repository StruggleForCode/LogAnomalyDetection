...
def TCN(train_x, train_y, valid_x, valid_y):
    ...
    # 计算 end_step：每个 epoch 的步数 * 总 epoch 数
    end_step = np.ceil(num_train_samples / batch_size) * epochs

    pruning_params = {
        'pruning_schedule': tfmot.sparsity.keras.PolynomialDecay(
            initial_sparsity=0.0,  # 初始不剪枝
            final_sparsity=0.5,  # 最终达到 50% 的稀疏率
            begin_step=0,
            end_step=end_step
        )
    }

    # 使用 prune_low_magnitude 将模型包装为剪枝模型
    ...
    # 训练剪枝模型
    pruned_model.fit(train_x, train_y, batch_size=batch_size, epochs=epochs,
                     validation_data=(valid_x, valid_y), callbacks=callbacks, verbose=2)

    # 训练完成后，剥离剪枝包装得到最终模型
    final_model = tfmot.sparsity.keras.strip_pruning(pruned_model)

    # 保存最终模型（建议使用 .keras 原生格式）
    final_model.save('./model/E-TCN_pruned.keras')
