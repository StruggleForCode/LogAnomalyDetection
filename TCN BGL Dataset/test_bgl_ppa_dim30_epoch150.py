...
# 设置日志记录
log_file = './log/test_ppa_dim30_epoch150_output.log'  # 日志文件路径
...
with open('./data/bgl_semantic_vec.json') as f:
    ...
    # Step1-2 PCA: 使用主成分分析进行数据降维，降到30维
    from sklearn.decomposition import PCA  # 导入PCA模块
    estimator = PCA(n_components=30)  # 创建PCA模型，设置降维后的维度为30
    ...
    pca = PCA(n_components=30)  # 创建PCA模型（再次用于去均值后的数据降维）
...
def read_test(split = 0.7):
    ...
    for i in range(0, len(logs_data)):
        padding = np.zeros((300, 30))  # 初始化一个300x30的零矩阵用于填充
...
def test_model(test_x):
    model = load_model('./model/E-TCN-PPA-DIM30-EPOCH150.h5')  # 加载训练好的模型
...
print('E-TCN-PPA-DIM30-EPOCH150 ')
# 输出TP, FP, TN, FN值
...
datas.to_csv('./result/E-TCN-PPA-DIM30-EPOCH150', index=False, header=False)