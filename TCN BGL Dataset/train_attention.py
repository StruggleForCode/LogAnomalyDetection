...
class Attention(Layer):
    ...
    def build(self, input_shape):
        # 创建一个可训练的权重矩阵 W，形状为 (features, 1)
        self.W = self.add_weight(name='att_weight',
                                 shape=(input_shape[-1], 1),
                                 initializer='normal',
                                 trainable=True)
        # 创建一个可训练的偏置向量 b，形状为 (time_steps, 1)
    ...
    def call(self, x):
        ...
        """
        # 计算注意力得分，公式：score = tanh(x * W + b)
        e = K.tanh(K.dot(x, self.W) + self.b)  # 形状为 (batch_size, time_steps, 1)
        # 对每个时间步的得分做 softmax，得到注意力权重
        a = K.softmax(e, axis=1)               # 形状为 (batch_size, time_steps, 1)
        # 对输入 x 的每个时间步加权求和
        output = K.sum(x * a, axis=1)           # 形状为 (batch_size, features)
        return output
...
def TCN_with_Attention(train_x, train_y, valid_x, valid_y):
    """
    构建、训练并保存带有注意力机制的 TCN 模型，用于日志异常检测。
    ...
    # 使用自定义注意力层替代 GlobalAveragePooling1D
    # 注意：原来的 x 形状为 (None, 300, 3)
    x = Attention()(x)  # 输出形状变为 (None, 3)

    # 全连接层输出二分类结果
    x = Dense(2, activation='softmax')(x)

    # 构建模型（注意使用新版的关键字：inputs 和 outputs）
    model = Model(inputs=inputs, outputs=x)
    model.summary()
    ...
    model.fit(train_x, train_y, batch_size=64, epochs=100, verbose=2, validation_data=(valid_x, valid_y))
    model.save('./model/E-TCN_with_Attention.keras')