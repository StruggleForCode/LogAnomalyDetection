...
with open('./data/hdfs_semantic_vec.json') as f:       # 以只读方式打开包含语义向量的 JSON 文件
    ...
    estimator = PCA(n_components=25)                     # 创建 PCA 实例，将目标降维后的维数设为25
    ...
    pca = PCA(n_components=25)                           # 再次创建一个 PCA 实例（此处可视为对中心化数据的再处理）
...
def read_test(path):                                   # 定义函数 read_test，参数 path 为测试数据文件路径
    ...
    for i in range(0, len(logs_data)):                 # 遍历每一条日志数据
        padding = np.zeros((300, 25))                  # 初始化一个 300x25 的全零矩阵，用作日志数据的填充（300个时间步，每个步长25维）
    ...
    text_x = np.reshape(train_x, (train_x.shape[0], train_x.shape[1], 25))  # 将输入特征调整为 (样本数, 300, 25) 的形状
...
def test_model(test_x):                              # 定义函数 test_model，参数 test_x 为测试数据的特征
    model = load_model('./model/E_TCN_GAP_PCA_dim25.h5')       # 加载预先训练好的模型文件
...
datas.to_csv('./result/result_HDFS_PCA_25', index=False, header=False)  # 将结果保存为 CSV 文件，不包含索引和表头