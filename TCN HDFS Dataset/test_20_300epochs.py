...
def test_model(test_x):                              # 定义函数 test_model，参数 test_x 为测试数据的特征
    model = load_model('./model/E_TCN_GAP_dim20_epochs_300.h5')       # 加载预先训练好的模型文件
...
datas.to_csv('./result/result_HDFS_PPA_Dim20_epochs300', index=False, header=False)  # 将结果保存为 CSV 文件，不包含索引和表头